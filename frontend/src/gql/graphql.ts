...
export type GetProjectQuery = {
	getProject: {
		user: { id: number; email: string; name: string; lastProjectId: number }
		project: { name: string }
		nodes: Array<{
			id: number
			name: string
...
export type GetValuesQuery = {
	getValues: Array<{ id: number; node_id: number }>
}
...
export function getSdk<C, E>(requester: Requester<C, E>) {
	return {
		...
		Register(
			...
			options?: C
		): Promise<ExecutionResult<RegisterMutation, E>> {
			return requester<RegisterMutation, RegisterMutationVariables>(
				RegisterDocument,
				variables,
				options
			) as Promise<ExecutionResult<RegisterMutation, E>>
		},
		Login(
			variables: LoginMutationVariables,
			options?: C
		): Promise<ExecutionResult<LoginMutation, E>> {
			return requester<LoginMutation, LoginMutationVariables>(
				LoginDocument,
				variables,
				options
			) as Promise<ExecutionResult<LoginMutation, E>>
		},
		Logout(
			variables?: LogoutMutationVariables,
			options?: C
		): Promise<ExecutionResult<LogoutMutation, E>> {
			return requester<LogoutMutation, LogoutMutationVariables>(
				LogoutDocument,
				variables,
				options
			) as Promise<ExecutionResult<LogoutMutation, E>>
		},
		ValuesUpdated(
